...
.lang-toggle-wrap{display:flex;gap:6px;margin-top:10px;}
.lang-btn{padding:4px 14px;border-radius:20px;font-size:11px;font-weight:800;
  cursor:pointer;border:2px solid rgba(255,255,255,.5);background:rgba(255,255,255,.15);
  color:white;transition:all .2s;}
.lang-btn.active{background:white;color:var(--acc,#1d4ed8);border-color:white;}
...
{{
  "english": "{target_lang_name}文（複数文はスペースで区切る）",
  "english_jp": "自然な日本語訳",
...
def build_context_prompt(context, src_label, is_biz, lang, level_key):
    ...
    return f"""
{src_label}のテキストを要約し、{target}学習コンテンツをJSONのみで作成してください（コードブロック不要）。
[テキスト]: {context[:2000]}
[場面]: {"Business/Fachmesse" if is_biz else "Alltag"}
[レベル]: {level_inst}
{{
  "english": "{target}文", "english_jp": "日本語訳",
  "chunked": "チャンク区切り",
  "grammar": "文法解説（日本語）",
  "vocab": {{"単語": "意味"}},
...
defaults = {"script_data":None,"chat_history":[],"saved_list":[],
            "voice_text":"","gen_count":0,"language":"en",
            "url_text_cache":""}
...
""", unsafe_allow_html=True)
    if not api_key:
        ...
        if mk: api_key = mk; st.markdown('<div style="color:#4ade80;font-size:12px;font-weight:700;">✅ APIキー設定済み</div>', unsafe_allow_html=True)
...
① モード＆レベルを選択<br>② 言語トグルで切替<br>③ 日本語を入力<br>④「生成する」→ 各タブで学習！<br>⑤ 気に入ったら📚に保存
</div>
<hr style="border-color:rgba(255,255,255,.1);margin:14px 0;">
...
# ============================================================
# TAB 1: INPUT ── ①例文削除済み ②音声入力 ③PDF ④URL ⑦レベル
# ============================================================
with tab1:
    # レベル選択
    level_key = st.selectbox("📊 学習レベル", list(LEVELS.keys()), index=1,
                             help="生成されるスクリプトの難易度を選択")
    ...
    scenario = ""
    if not is_biz:
        scenario_dict = DAILY_SCENARIOS[lang]
        scenario = st.selectbox("🎬 シナリオ", list(scenario_dict.keys()))
    ...
    if im == "✏️ テキスト":
        placeholder_txt = {
            ('en', True): "例: 弊社のセンサーは従来品より30%省エネです。",
            ('en', False): "例: コーヒーを1杯ください。",
            ('de', True): "例: Unser Sensor spart 30% mehr Energie.",
            ('de', False): "例: Ich hätte gerne einen Kaffee.",
        }.get((lang, is_biz), "日本語を入力してください")
        user_input = st.text_area(
            "スクリプトにしたい内容を入力（日本語でOK）",
            placeholder=placeholder_txt, height=100, key="text_input_main")
...
<div style="background:{C['light']};border:1px solid {C['border']};border-radius:14px;
  ...
  <div style="font-size:11px;color:#64748b;">録音ボタンを押して話してください。Geminiが文字起こしします。</div>
...
<div style="background:{C['light']};border:1px solid {C['border']};border-radius:14px;
  ...
  <div style="font-size:11px;color:#64748b;">会社HP・製品ページなどのURLを入力してください。</div>
    ...
    if st.button("✨ スクリプト＆学習コンテンツを生成する", type="primary",
        ...
        if not api_key: st.error("❌ APIキーが設定されていません。")
        ...
        elif not mdl: st.error("❌ モデルの初期化に失敗しました。")
        else:
            with st.spinner(f"AIが{LS['name']}スクリプトを作成中... ✨"):
                try:
                    ...
                    err = str(e)
                    if "404" in err: st.error("❌ モデルエラー。APIキーを確認してください。")
                    elif "403" in err: st.error("❌ 認証エラー。APIキーを再確認してください。")
                    else: st.error(f"❌ エラー: {err}")


# ── 表示用ヘルパー ────────────────────────────────────────────
def d_lang(data):
    """データが持つ言語設定を返す"""
    return data.get('_lang', 'en') if data else lang
...
with tab2:
    if not data:
        ...
        dl = d_lang(data); DC = ac(is_biz, dl); DLS = LANG[dl]
        lv = data.get("_level","")
        st.markdown(f"""
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
  <span style="font-size:11px;color:#94a3b8;">{DLS['flag']} {DLS['name']} ｜ レベル: <strong style="color:{DC['main']};">{lv}</strong></span>
</div>
        ...
        with col_save:
            if st.button("📚 マイ学習帳に保存", use_container_width=True, key="save_btn"):
                item = {
                    ...
                    "level": data.get("_level",""), "mode": data.get("_mode",""),
...
<div class="ep-card">
  ...
  <div style="font-size:13px;color:#475569;line-height:1.8;">{data.get('grammar','')}</div>
        ...
        if data.get('vocab'):
            v_html = "".join([f'<span class="ep-vocab" style="background:{DC["light"]};color:{DC["main"]};"><strong>{k}</strong>: {v}</span>'
        ...
        if is_biz and data.get('qa_pairs'):
            qa_html = "".join([f'<div class="ep-qa"><div style="font-size:13px;font-weight:600;color:#1e293b;">{q.get("question","")}</div><div style="font-size:11px;color:#94a3b8;">{q.get("question_jp","")}</div><div style="font-size:11px;color:{DC["main"]};font-weight:600;margin-top:5px;">💡 {q.get("hint","")}</div></div>'
                               for q in data['qa_pairs']])
            st.markdown(f'<div class="ep-card"><div class="ep-label" style="background:{DC["main"]};">🙋 想定Q&A</div>{qa_html}</div>', unsafe_allow_html=True)

        st.markdown(f'<div class="ep-tip">💡 <strong>チャンク読みのコツ：</strong>スラッシュで区切り、<strong>左から順番に</strong>意味をつかみながら読みましょう！</div>', unsafe_allow_html=True)
...
<div style="background:#fffbeb;border:1px solid #fde68a;border-radius:14px;padding:16px;margin-top:14px;">
  ...
  <div style="display:flex;flex-direction:column;gap:8px;">
    {"".join([f'<div style="display:flex;gap:10px;align-items:flex-start;font-size:12px;color:#78350f;"><span style="background:{DC[chr(39)+"main"+chr(39)]};color:white;width:20px;height:20px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:10px;flex-shrink:0;">{n}</span><span>{t}</span></div>' for n,t in [("1","<strong>0.8x</strong> → 音声を聴きながら口をパクパク（オーバーラッピング）"),("2","<strong>1.0x</strong> → 声に出して一緒に読む（シャドーイング）"),("3","<strong>1.2x</strong> → スクリプトを見ずに追いかける（スピードトレーニング）")]])}
  </div>
</div>""".replace("chr(39)+\"main\"+chr(39)", '"main"'), unsafe_allow_html=True)
...
<div style="background:{DC['light']};border:1px solid {DC['border']};border-radius:16px;padding:18px;margin-bottom:14px;">
  ...
  <div style="font-size:12px;color:#64748b;">🇯🇵 {data.get('english_jp','')}</div>
        ...
        st.markdown(f'<div style="font-size:12px;font-weight:700;color:#334155;margin-bottom:8px;">🎤 マイクで音読（録音後に自動採点）</div>', unsafe_allow_html=True)
...
with tab5:
    if not data:
        ...
        dl = d_lang(data); DC = ac(is_biz, dl); DLS = LANG[dl]
        bq = data.get('blank_q',''); ba = data.get('blank_a',''); hint = data.get('hint','')
        st.markdown(f"""
<div style="background:{DC['light']};border:1px solid {DC['border']};border-radius:16px;padding:18px;margin-bottom:14px;">
...
with tab6:
    if not data:
        ...
        if not st.session_state.chat_history:
            ...
            opener = openers.get((dl, is_biz), "Hello!")
        ...
        if user_msg and mdl:
            ...
            with st.spinner("AIが返答中..."):
                ...
                if show_hint: cp += "\nAntwort auf Deutsch/英語で返答し、その後（日本語訳）を添えてください。"
                cp += f"\nNachricht des Lernenden/ユーザー: {user_msg}"
                try:
        ...
        with c_d:
            if st.button("📈 少し難しく話しかける", key="bump_diff"):
                if mdl:
                    with st.spinner():
                        try:
                            bp = f"{'Verwenden Sie' if dl=='de' else 'Use'} etwas schwierigere Ausdrücke {'auf Deutsch' if dl=='de' else 'in English'} und stellen Sie eine neue Frage. Kontext: {data.get('english','')}"
...
<div style="background:linear-gradient(135deg,#1e293b,#334155);color:white;
  ...
  <div style="display:inline-block;background:rgba(255,255,255,.2);border-radius:20px;
    ...
    if not saved:
        ...
        export_data = json.dumps([{"title":s["title"],"script":s["english"],"jp":s.get("english_jp",""),
                                   "level":s["level"],"lang":s.get("lang","en"),"saved_at":s["saved_at"]} for s in saved],
                                 ensure_ascii=False, indent=2)
        st.download_button("⬇️ 学習リストをJSONでダウンロード", data=export_data,
                           file_name="pitch_saved.json", mime="application/json")
        st.markdown("---")
        for item in reversed(saved):
            il = item.get("lang","en"); IC = ac(item.get("mode")=="business", il)
            ILS = LANG[il]
            st.markdown(f"""
<div class="save-card">
...
# ── フィラーカード ─────────────────────────────────────────
if is_biz and data:
    dl = d_lang(data); DC = ac(True, dl); DLS = LANG[dl]
    fcard_bg = "#eff6ff" if dl=='en' else "#fffbeb"
    fcard_bdr = "#bfdbfe" if dl=='en' else "#fde68a"
    fcard_tx  = "#1d4ed8" if dl=='en' else "#b45309"
    fc = "".join([
        f'<div class="ep-filler-card" style="background:{fcard_bg};border:1px solid {fcard_bdr};">'
        f'<div class="ep-filler-en" style="color:{fcard_tx};">{ph_}</div>'
        f'<div class="ep-filler-jp">{jp}</div></div>'
        for ph_,jp in FILLERS[dl]
    ])
    st.markdown(f"""
