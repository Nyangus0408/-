...
.stTabs [data-baseweb="tab-list"] {
    ...
    gap: 0; padding: 0 8px; position: sticky; top: 0;
    z-index: 100; box-shadow: 0 2px 8px rgba(0,0,0,.06);
}
.stTabs [data-baseweb="tab"] {
    font-weight: 700 !important; font-size: 12px !important;
    padding: 12px 14px !important; border-radius: 0 !important; color: #94a3b8 !important;
}
...
.stButton > button[kind="primary"] { background: var(--accent, #1d4ed8) !important; }

.stTextArea textarea, .stTextInput input {
    border-radius: 12px !important; border: 2px solid #e5e7eb !important;
    font-family: inherit !important; transition: border-color .2s !important;
}
.stTextArea textarea:focus, .stTextInput input:focus {
    border-color: var(--accent, #1d4ed8) !important; box-shadow: none !important;
}
...
.ep-card {
    ...
    margin: 12px 0; box-shadow: 0 2px 12px rgba(0,0,0,.07); border: 1px solid #e8edf5;
}
.ep-script { border-radius: 16px; padding: 20px; margin-bottom: 16px; border-left-width: 6px; border-left-style: solid; }
...
.ep-vocab { border-radius: 20px; padding: 4px 12px; font-size: 12px; font-weight: 600; display: inline-block; margin: 3px; }
.ep-qa { background: #f8fafc; border-radius: 12px; padding: 14px; margin-bottom: 10px; }
.ep-tip { background: #fffbeb; border: 1px solid #fde68a; border-radius: 14px; padding: 16px 18px; margin: 14px 0; font-size: 13px; line-height: 1.7; }
...
.ep-bar-wrap { background: #f1f5f9; border-radius: 8px; height: 12px; overflow: hidden; margin-bottom: 16px; }
...
.ep-chat-wrap { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 16px; padding: 16px; min-height: 220px; max-height: 360px; overflow-y: auto; margin-bottom: 12px; }
...
.ep-filler-card { background: #eff6ff; border: 1px solid #bfdbfe; border-radius: 10px; padding: 8px 14px; }
...
.ep-ph-icon { font-size: 52px; margin-bottom: 14px; }
.ep-ph-title { font-size: 15px; font-weight: 700; margin-bottom: 6px; color: #64748b; }
.ep-ph-sub { font-size: 12px; }

.ep-alert-green { background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 12px; padding: 12px 16px; text-align: center; color: #16a34a; font-weight: 700; font-size: 13px; margin-top: 14px; }
.ep-alert-orange { background: #fff7ed; border: 1px solid #fed7aa; border-radius: 12px; padding: 12px 16px; text-align: center; color: #ea580c; font-size: 12px; margin-top: 14px; }
</style>
...
def ac(is_biz):
    ...
    return {"main": "#0d9488", "light": "#f0fdfa", "border": "#99f6e4"}
...
<div style="display:flex;gap:8px;">
  <button onclick="document.getElementById('epAudio').playbackRate=0.8;document.getElementById('epAudio').play();"
    style="flex:1;padding:10px 0;border:2px solid #e2e8f0;border-radius:10px;background:white;cursor:pointer;font-weight:700;font-size:12px;">
    🐢 0.8x<br><span style="font-size:10px;opacity:.7;">ゆっくり</span></button>
  <button onclick="document.getElementById('epAudio').playbackRate=1.0;document.getElementById('epAudio').play();"
    style="flex:1;padding:10px 0;border:2px solid #e2e8f0;border-radius:10px;background:white;cursor:pointer;font-weight:700;font-size:12px;">
    ▶️ 1.0x<br><span style="font-size:10px;opacity:.7;">標準</span></button>
  <button onclick="document.getElementById('epAudio').playbackRate=1.2;document.getElementById('epAudio').play();"
    style="flex:1;padding:10px 0;border:2px solid #e2e8f0;border-radius:10px;background:white;cursor:pointer;font-weight:700;font-size:12px;">
    ⚡ 1.2x<br><span style="font-size:10px;opacity:.7;">速め</span></button>
</div>"""
...
<div class="ep-placeholder">
  ...
  <div class="ep-ph-sub">英文が生成されると {tab_name} が表示されます</div>
</div>"""
...
""", unsafe_allow_html=True)
    ...
    if not api_key:
        manual_key = st.text_input("🔑 Gemini API Key", type="password", placeholder="AIzaSy...")
        if manual_key:
            api_key = manual_key
            st.markdown('<div style="color:#4ade80;font-size:12px;font-weight:700;">✅ APIキー手動設定済み</div>', unsafe_allow_html=True)
        else:
            st.markdown('<div style="color:#fbbf24;font-size:12px;font-weight:700;">⚠️ APIキーが未設定です</div>', unsafe_allow_html=True)
    else:
        st.markdown('<div style="color:#4ade80;font-size:12px;font-weight:700;">✅ APIキー自動連携済み (Secrets)</div>', unsafe_allow_html=True)
...
<div style="font-size:12px;color:rgba(255,255,255,.75);line-height:2.2;">
① モードを選択<br>② 日本語テキストを入力<br>③「生成する」ボタン<br>④ 各タブで学習！
</div>
<hr style="border-color:rgba(255,255,255,.1);margin:16px 0;">
...
# SESSION STATE
# ============================================================
for k, v in [("script_data", None), ("chat_history", []), ("sel_example", "")]:
...
<div style="background:linear-gradient(135deg,{C['main']},{C['main']}cc);color:white;
     padding:20px 24px 18px;border-radius:16px;margin-bottom:4px;
  ...
  <div style="display:inline-block;background:rgba(255,255,255,.2);border:1px solid rgba(255,255,255,.4);
       border-radius:20px;padding:4px 14px;font-size:11px;font-weight:700;margin-top:10px;">
...
sys_prompt = (
    "あなたはビジネス英語の専門家です。中学レベルのSVO文法を使い、展示会で伝わるシンプルな英文を作成してください。1文は必ず12単語以内。"
    if is_biz else
    "あなたは日常英会話のコーチです。旅行・カフェ・自己紹介で使えるシンプルで温かみのある英文を作成してください。1文は必ず12単語以内。"
)

EXAMPLES = {
    True:  ["このセンサーは従来品の半分の電力で動作します", "弊社の新製品は防水・防塵性能を備えています",
            "納期は2週間です。大量注文も対応可能です", "このシステムは導入コストを30%削減できます"],
    False: ["コーヒーを1杯ください", "駅への行き方を教えてください",
            "田中と申します。よろしくお願いします", "おすすめのランチは何ですか？"]
}
...
""", unsafe_allow_html=True)
    ...
    for i, ex in enumerate(EXAMPLES[is_biz]):
        with ex_cols[i % 2]:
            if st.button(ex[:18] + "…", key=f"ex_{i}", use_container_width=True):
                st.session_state.sel_example = ex
                st.rerun()
    ...
    user_input = st.text_area(
        "英語にしたい日本語を入力",
        value=st.session_state.sel_example,
        placeholder="例: 弊社のセンサーは従来品の半分の電力で動作します。",
        height=110, key="main_input"
    )
...
"""
                try:
                    ...
                    if "404" in err or "not found" in err.lower():
                        st.error("❌ モデルエラー: APIキーまたはGeminiのモデル名を確認してください。")
                    elif "403" in err or "PERMISSION" in err:
                        st.error("❌ 認証エラー: APIキーが正しくありません。Google AI Studioで再発行してください。")
                    else:
                        st.error(f"❌ エラー: {err}")
...
<div class="ep-script" style="background:{C['light']};border:2px solid {C['border']};border-left-color:{C['main']};">
  ...
  <div style="font-size:11px;color:#94a3b8;margin-top:8px;">/ はチャンク（意味の塊）の区切りです。左から順番に理解しましょう。</div>
</div>
""", unsafe_allow_html=True)
...
""", unsafe_allow_html=True)
        ...
        st.markdown(f"""
<div class="ep-tip">
  💡 <strong>チャンク読みのコツ：</strong>スラッシュ（/）の前で少し間を取り、
  <strong>左から順番に</strong>意味をつかみながら読みましょう。
  日本語のように後ろから返り読みするのはNGです！
</div>
""", unsafe_allow_html=True)
...
<div style="background:#fffbeb;border:1px solid #fde68a;border-radius:16px;padding:18px;margin-top:16px;">
  ...
  <div style="display:flex;flex-direction:column;gap:10px;">
    <div style="display:flex;gap:12px;align-items:flex-start;font-size:12px;color:#78350f;line-height:1.6;">
      <span style="background:{C['main']};color:white;width:22px;height:22px;border-radius:50%;
            display:flex;align-items:center;justify-content:center;font-weight:900;font-size:11px;flex-shrink:0;">1</span>
      <span><strong>0.8x</strong> → 音声を聴きながら口をパクパクさせる（オーバーラッピング）</span>
    </div>
    <div style="display:flex;gap:12px;align-items:flex-start;font-size:12px;color:#78350f;line-height:1.6;">
      <span style="background:{C['main']};color:white;width:22px;height:22px;border-radius:50%;
            display:flex;align-items:center;justify-content:center;font-weight:900;font-size:11px;flex-shrink:0;">2</span>
      <span><strong>1.0x</strong> → 声に出して一緒に読む（シャドーイング）</span>
    </div>
    <div style="display:flex;gap:12px;align-items:flex-start;font-size:12px;color:#78350f;line-height:1.6;">
      <span style="background:{C['main']};color:white;width:22px;height:22px;border-radius:50%;
            display:flex;align-items:center;justify-content:center;font-weight:900;font-size:11px;flex-shrink:0;">3</span>
...
""", unsafe_allow_html=True)
        ...
        st.markdown('<div style="font-size:12px;font-weight:700;color:#334155;margin-bottom:8px;">🎤 マイクで音読する（録音後に自動採点）</div>', unsafe_allow_html=True)
...
{{
  ...
  "good_words": ["正しく言えた単語のリスト"],
  "bad_words": ["うまく言えなかった単語のリスト"],
  "feedback": "日本語でのワンポイントフィードバック（1文）"
}}
"""
                try:
...
<div class="ep-card" style="margin-top:16px;">
  ...
  <div class="ep-bar-wrap">
    <div class="ep-bar" style="width:{score}%;background:{score_color};"></div>
  </div>
  <div style="margin-bottom:12px;">{good_chips}{bad_chips}</div>
  <div style="font-size:11px;color:#94a3b8;">認識テキスト: "{rd.get('transcript','')}"</div>
  {f'<div style="font-size:12px;color:#475569;margin-top:8px;padding:8px;background:#f8fafc;border-radius:8px;">💬 {rd.get("feedback","")}</div>' if rd.get('feedback') else ''}
</div>
""", unsafe_allow_html=True)
...
with tab5:
    if not data:
        ...
        fb = data.get('fill_blank') or {"sentence": data.get('blank_q',''), "answer": data.get('blank_a',''), "hint": data.get('hint','')}
...
<div style="background:{C['light']};border:1px solid {C['border']};border-radius:16px;padding:20px;margin-bottom:16px;">
  ...
  <div style="background:white;border:2px dashed #cbd5e1;border-radius:14px;padding:20px;text-align:center;margin-bottom:16px;">
    <div style="font-size:19px;font-weight:700;color:#1e293b;">{fb.get('sentence', data.get('blank_q',''))}</div>
    <div style="font-size:12px;color:#94a3b8;margin-top:8px;">💡 ヒント: {fb.get('hint', data.get('hint',''))}</div>
  </div>
</div>
""", unsafe_allow_html=True)
        ...
        user_ans = st.text_input("答えを入力してください", placeholder="英単語を入力...", key="prac_ans")

        col_check, col_audio = st.columns(2)
        with col_check:
            check_btn = st.button("✅ 答え合わせ", use_container_width=True, type="primary", key="check_btn")
        with col_audio:
            play_btn = st.button("🔊 正解の文を聴く", use_container_width=True, key="play_btn")

        correct_ans = fb.get('answer', data.get('blank_a', ''))
...
<div style="background:white;border:1px solid #e2e8f0;border-radius:16px;padding:20px;text-align:center;margin-top:12px;">
  ...
  <div style="font-size:28px;font-weight:900;color:#22c55e;margin-bottom:10px;">{correct_ans}</div>
  <div class="ep-alert-green">🎉 正解！素晴らしい！</div>
</div>
""", unsafe_allow_html=True)
            else:
                st.markdown(f"""
...
with tab6:
    if not data:
        ...
        if not st.session_state.chat_history:
            opener = (
                "Hello! I'm visiting your booth today. Tell me about your product."
                if is_biz else
